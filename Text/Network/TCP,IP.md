...
- 네트워크의 어떤 계층에서 계층으로 데이터가 전달 될 때 한 덩어리 단위를 PDU(Protocol Data Unit)이라고 하며 제어 관련 정보들이 포함된 헤더와 데이터를 의미하는 페이로드로 구성 되어 있으며 계층 별로 부르는 명칭이 다릅니다.
...
- 각 계층 별로 처리 할 수 있는 기기가 다릅니다. 상위계층의 기기는 하위계층을 모두 처리가능 하지만 그 반대는 불가능 합니다.
...
- L7 스위치는 로드밸런서 라고도 하며 서버의 부하를 분산하는 기기 입니다. 클라이언트로 오는 요청을 뒤의 여러 서버로 나누는 역할을 합니다.

- URL, 서버, 캐시, 쿠키들을 기반으로 트래픽을 분산 시킵니다. 

<br>

#### 헬스체크

- 정상적인 서버임을 판별하기위해 전송주기, 재전송 회수등을 설정한 이후 반복적으로 서버에 요청을 보내는 것을 말합니다.

<br>

#### 로드밸런서 서버 이중화

- 서비스를 안정적으로 운용하기 위해서는 2대 이상의 서버가 필요합니다. 로드밸런서는 2대 이상의 서버를 기반으로 가상 IP를 제공, 이를 기반으로 
안정적으로 서비스 합니다.
...
- 라우터는 여러 개의 네트워크를 연결, 분할, 구분 시켜주는 역할을 하며 다른 네트워크에 존재하는 장치끼리 데이터를 주고 받을 때 패킷 소모를 최소화 하고 경로를 최적화 하여 최소 경로로 패킷을 포워딩하는 장비 입니다.
...
- L2, L3 스위치의 기능을 모두 갖춘 장비를 말합니다. L3 스위치를 라우터라고 하여도 무방 합니다. 소프트웨어 기반 라우팅과, 하드웨어 기반 라우팅으로 나누어지고 하드웨어를 담당하는 장치를 L3 스위치라고 합니다.
...
- L2 스위치는 장치들의 MAC 주소를 MAC 주소 테이블을 통해 관리하며 연결 된 장치로 부터 패킷이 왔을 때 패킷 전송을 담당 합니다.

- IP 주소는 이해 하지 못해 IP 주소 기반 라우팅이 불가능 하며, 패킷의 MAC 주소를 읽어 스위칭 하는 역할을 합니다.

<br>

#### 브리지

- 브리지는 두개의 근거리 통신망(LAN)을 상호 접속 가능하도록 MAC 주소를 MAC 주소 테이블로 관리하는 포트와 포트의 다리 역할을 하는 장치 입니다.

<br>

### IP 주소

- 인터넷 계층에서 사용하는 IP 주소에 대해 알아봅시다.

- 컴퓨터 간의 통신에 IP 주소 기반으로 통신하는 것으로 알고 있지만, 정확하게는 IP 주소의 ARP를 통하여 MAC주소를 찾아 MAC주소 기반으로 통신을 합니다.

- ARP(Address Resolution Protocol)란 IP 주소로 부터 MAC 주소를 구하는 다리역할을 하는 프로토콜 입니다.

- 가상 주소인 IP를 실제 MAC 주소로 변환 할 수 있습니다. 반대로 RARP는 실제 MAC 주소를 가상 IP 주소로 변환 하기도 합니다.

<br>

  - 브로드 캐스트: 송신 호스트가 전송한 데이터가 네트워크에 연결 된 모든 호스트에 전송되는 방식.
...
- IP 주소는 IPv4 & Ipv6 로 나뉩니다.

- IPv4는 32비트를 8비트 단위 점을 찍어 표기하며, 111.22.33.45 등의 방식으로 주소를 표기합니다.

- IPv6는 64비트를 16비트 단위로 점을 찍어 표기하며, 1001:ab1::ff00:42:1234 등의 방식으로 주소를 표기합니다.

- 추세는 v6로 옮겨지고 있으나 가장 많이 사용되고 있는 것은 v4 입니다.